...
class S3Reader:
    ...
    def _init_s3_client(self):
        """Initialize boto3 S3 client with retry configuration"""
        try:
            ...
            self.s3_client = boto3.client('s3', config=retry_config, **boto3_config)
...
"""
    ...
    def path_exists(self, s3_path: str) -> bool:
        """
        Check if S3 path exists (has objects).
        ...
        """
        if self.use_mock:
            self.logger.info("✅ MOCK mode - path considered existing")
            return True
        
        try:
            bucket, prefix = self._parse_s3_path(s3_path)
            
            # List objects with limit of 1 to check existence
            response = self.s3_client.list_objects_v2(
                Bucket=bucket,
                Prefix=prefix,
                MaxKeys=1
            ...
            if exists:
                self.logger.info(f"✅ S3 path exists: {s3_path}")
            else:
                self.logger.warning(f"❌ S3 path not found: {s3_path}")
            return exists
    ...
    def get_object_count(self, s3_path: str) -> int:
        ...
        Returns:
            Number of objects at path
        """
        if self.use_mock:
            return 1  # Mock returns 1 object
        
        try:
            bucket, prefix = self._parse_s3_path(s3_path)
            
            # Use paginator for accurate count
            paginator = self.s3_client.get_paginator('list_objects_v2')
